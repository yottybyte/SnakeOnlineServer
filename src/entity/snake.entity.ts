...
export class SnakeEntity implements IStepGameLoop {
  ...
  public spawnBullet() {
    ...
    this.room.addLoopEntity(new BulletEntity(this.room, snakeHead.x, snakeHead.y, this.direction, this.id, this.color));
  }
  ...
  public gameStep(time: number) {
    ...
    if (time % 150 === 0 && !this.isSpeedBonusActive()) {
      this.move();
    } else if (time % 75 === 0 && this.isSpeedBonusActive()) {
      this.move();
    }
  }
